...
class VertexArray{
    ...
    */
    ...
    pushVector(other){
        if(!(other instanceof VertexArray)){
    ...
    crossProduct3D(other) {
        ...
        const ab = new VertexArray(new Vector(b.x - a.x, b.y - a.y, b.z - a.z));
        const ac = new VertexArray(new Vector(c.x - a.x, c.y - a.y, c.z - a.z));
        ...
        return new VertexArray(new Vector(x, y, z));
    }
    ...
    plus(other) {
        ...
        } else if(other instanceof VertexArray && this.space === other.space){
    ...
    static sum(first, second) {
        if(first instanceof VertexArray){
...
class Figure{
    ...
    constructor (name = "figure", colorFilling = "#fff", colorBorder = null, transforms = null, ...vertexes){

        if(vertexes.length === 0 || (Array.isArray(vertexes) && vertexes[0].length === 0)){
            vertexes = vertexes[0];
        }
        ...
        this.#name = (name === "figure") ? "figure" : "figure " + name;
    ...
    trasform(...transforms){
        transforms.forEach(callBack => {
            this.vertexes = this.transformVertexes(this.vertexes, callBack);
        });
    }
    ...
    checkVertexVector(args){
        ...
        if(Array.isArray(args) && args.length === 1){
            if(args[0].length === 0){
                return new VertexArray(2);
            }

            while((args.length === 1) && !((args[0] === undefined && args.at(0) instanceof Vector))){
                args = args[0];
            }
            if(args instanceof VertexArray){
                return args;
            }
            vertexes = new VertexArray(args[0].length);
    ...
    transformVertexes(vertexVector, callBack){
        let newVertexVector = new VertexArray(vertexVector.space);
...
class Triangle extends Figure{
    ...
    constructor(identificators, color = "#fff", border = null, transforms, ...vertexes){
        ...
        if(vertexes.length === 0){
            this.vertexes = new VertexArray([1, 0], [0, 2], [2, 2]);
        }
...
class Square extends Figure{
    ...
    constructor(identificators, color = "#fff", border = null, transforms, ...vertexes){
        ...
        if(vertexes.length === 0){
            this.vertexes = new VertexArray([0, 0], [2, 0], [2, 2], [0, 2]);
        }
...
class tangram{
    ...
    constructor(...figures){
        ...
        this.#vertexes = new VertexArray(...figures);
...
let V0 = new VertexArray(P0, new Vector(103, 197), [62, 140]);
let V1 = new VertexArray([131,3], [159,43], [103,43]);
let V2 = new VertexArray([103,83], [162,83], [102,163]);
